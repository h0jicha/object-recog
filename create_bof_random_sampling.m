...
function bof = create_bof_random_sampling(codebook, filenames)
    n = size(filenames, 1);
    ...
    for j = 1 : n
        ...
        p = create_random_points(I, 1000); % random sampling
        [fts, p2] = extractFeatures(I,p);

        % 各特徴点についての for-loop
        for i=1:size(p2,1)
            % 一番近いcodebook中のベクトルを探してindexを求める．
            [~, simIndex] = search_most_similar_vec(codebook, fts(i, :));
